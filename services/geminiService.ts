...
export const enhancePrompt = async (prompt: string): Promise<string> => {
  ...
  try {
    ...
    if (!response.ok) {
      const errorBody = await response.json().catch(() => ({ error: 'Failed to parse error response' }));
      console.error("Backend Proxy Error:", errorBody);
      throw new Error(`Failed to enhance prompt. Status: ${response.status}. ${errorBody.error || 'Unknown server error'}`);
    }
    ...
    if (data.enhancedPrompt) {
      ...
      throw new Error("Invalid response structure from backend. The response did not contain the expected enhanced prompt data.");
    }
  } catch (error: any) {
    console.error("Error during prompt enhancement via backend:", error);
    throw new Error(error.message || "An unexpected error occurred while enhancing prompt.");
  }
};
